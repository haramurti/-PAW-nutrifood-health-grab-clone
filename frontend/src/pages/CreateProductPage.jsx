...
function CreateProductPage() {
  ...
  const submitHandler = (e) => {
    ...
    if (Object.values(input).includes("")) {
      ...
      state.price = parseInt(String(input.price).replace(/\./g, ''));
    ...
    <div className="relative h-full">
      ...
      <div className="absolute bottom-0 w-full text-sm bg-white">
        <div className="flex gap-3 pt-3 pb-5 mx-4">
          ...
          <button
            className={`bg-[var(--primary)] w-1/3 rounded py-2 text-white ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            onClick={submitHandler}
            disabled={isSubmitting}
          >
            {isSubmitting ? "Sedang diproses... ⏳" : "Next"}
          </button>
        </div>
      </div>
    </div>
